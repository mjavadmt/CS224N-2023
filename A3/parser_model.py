...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # initialize input layer to hidden layer weights and bias with xavier
        ...
        # define hidden layer for EXAM 
        self.hidden_to_hidden_weight = nn.Parameter(torch.empty(self.hidden_size, self.hidden_size))
        self.hidden_to_hidden_bias = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.xavier_uniform_(self.hidden_to_hidden_weight)
        nn.init.uniform_(self.hidden_to_hidden_bias)

        # initialize hidden layer to output layer weight and bias with xavier
    ...
    def forward(self, w):
        ...
        z = torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        h = F.relu(z)
        h = self.dropout(h)
        ## add for EXAM 3
        other_inner = torch.matmul(h, self.hidden_to_hidden_weight) + self.hidden_to_hidden_bias
        other_inner = F.relu(other_inner)
        logits = torch.matmul(other_inner, self.hidden_to_logits_weight) + self.hidden_to_logits_bias

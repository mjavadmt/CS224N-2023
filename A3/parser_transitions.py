...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            curr_word = self.buffer.pop(0)
            self.stack.append(curr_word)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        batch_to_parse = unfinished_parses[:batch_size]
        transitions = model.predict(batch_to_parse)
        for i in range(len(transitions)):
            ith_sentece_dependencies = batch_to_parse[i].parse_step(transitions[i])
            if not batch_to_parse[i].buffer and len(batch_to_parse[i].stack) == 1:
                unfinished_parses.remove(batch_to_parse[i])
    dependencies = [parsed_transitions.dependencies for parsed_transitions in partial_parses]
    # END YOUR CODE

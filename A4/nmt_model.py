...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # for linear layer parameters : something you want to remain as your final(output) shape in out_features
        ...
        self.target_vocab_projection = nn.Linear(
            in_features=hidden_size, out_features=len(self.vocab.tgt), bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        output, input_sizes = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(output, (1, 0, 2))

        concatenated_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(concatenated_hidden)
        concatenated_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(concatenated_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for ith_time_word in torch.split(Y, 1):
            squeezed_word = torch.squeeze(ith_time_word, dim=0)
            Ybar_t = torch.cat((squeezed_word, o_prev), dim=1)

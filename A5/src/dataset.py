...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_idx = random.randint(4, int(self.block_size*7/8))
        document = document[:truncate_idx]

        quarter_length = int((1/4) * len(document))
        noise = random.randint(-int(quarter_length / 8), int(quarter_length / 8))
        masked_content_len = quarter_length + noise
        start_index = random.randint(0, quarter_length * 2)

        prefix = document[:start_index]
        masked_content = document[start_index:start_index+masked_content_len]
        suffix = document[start_index+masked_content_len:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        num_of_pads = self.block_size - len(masked_string)
        masked_string += self.PAD_CHAR * num_of_pads

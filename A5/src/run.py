...
if args.function == 'pretrain':
    ...
    # num_workers=4
    # writer=writer
    conf = {"max_epochs": 650, "batch_size": 120, "learning_rate": args.pretrain_lr, "lr_decay": True,
            "warmup_tokens": 512*20, "final_tokens": 200*len(pretrain_dataset)*block_size, "num_workers": 4, "writer": writer}

    tconf = trainer.TrainerConfig(**conf)
    ...
    conf = {"max_epochs": 75, "batch_size": 256, "learning_rate": 6e-4, "lr_decay": True,
            "warmup_tokens": 512*20, "final_tokens": 200*len(pretrain_dataset)*block_size, "num_workers": 4, "writer": writer}
    ...
    fine_tune_dataset = dataset.NameDataset(pretrain_dataset, open(
        args.finetune_corpus_path, encoding="utf-8").read())
    trainer = trainer.Trainer(model, fine_tune_dataset, None, tconf)
    trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)

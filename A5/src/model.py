...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C = nn.Parameter(torch.empty(
            1, config.bottleneck_dim, config.n_embd))
    ...
    def forward(self, x_input):
        ...
        # print(f"\nx shape : {x_input.shape}")
        C_normed = self.ln1(self.C)
        attention_applied = self.attn(x_input, C_normed)
        # print(f"\nattention shape: {attention_applied.shape}")
        x_input = attention_applied
        ...
        # attention_layer = self.attn(x_input, c_normed)
        # print(f"attention shape : {attention_layer.shape}")
        # linear_layer = self.mlp(self.ln2(attention_layer))
        # print(f"linear shape : {linear_layer.shape}")
        # linear_layer += attention_layer
        # return linear_layer
        # END YOUR CODE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        # Should be around 3-5 lines.
        # print(f"\ny shape: {y.shape}")
        # print(f"\nx shape in up projection : {x_input.shape}")
        y_normed = self.ln1(y)
        attention_applied = self.attn(y_normed, x_input)
        # print(f"\nattention shape in up projection: {attention_applied.shape}")
        y = x_input + attention_applied
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        b, t = idx.size()
        assert t <= self.block_size, "Cannot forward, model block size (%d, %d) is exhausted." % (
            t, self.block_size)
